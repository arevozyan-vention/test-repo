...
### Manual path (tools already installed)

Requires a running `docker`, plus `k3d`, `kubectl`, `terraform` >= 1.9, `task`, `mkcert`:

```sh
task up      # = cluster → certs → secrets → deploy
...
Down to a fully clean machine: `mkcert -uninstall && rm -rf "$(mkcert -CAROOT)"`; macOS - `colima delete -f`; WSL - `certutil.exe -user -delstore Root mkcert`, or radically - `wsl --unregister Ubuntu`.
